...
export async function GET() {
  try {
   ...
   //  console.log("Upserting data");
    ...
    // Load images
    const imageBuffer = fs.readFileSync(path.join(basePath, "sm_leaderboard.png")  );
    ...
    ctx.fillStyle = "black";
    ctx.fillText(  text.toUpperCase(), (baseImage.width - ctx.measureText(text.toUpperCase()).width) / 2,  85  );
    ctx.font = "35px sans-serif";
    ctx.fillText(formatDateRange().toUpperCase(), (baseImage.width - ctx.measureText(formatDateRange().toUpperCase()).width) / 2,  135  );
    ...
    const uploadServer =  previousMonthYear + "_" + Math.floor(Math.random() * 1000000) + ".png";
    ...
    return NextResponse.json(
      { data: {status: "success", message: "Image created successfully", path: upLoadPath + "/" + uploadServer,} },
      { status: 200 }
    );
  } catch (error) {
